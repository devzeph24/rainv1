...
/**
 * Test card details - used when USE_TEST_CARDS=true to avoid API costs
 */
export const TEST_CARD = {
  pan: '4549240609436532',
  cvc: '906',
  expirationMonth: '11',
  expirationYear: '2030',
  last4: '6532',
...
/**
 * Create a mock test card (used when USE_TEST_CARDS=true)
 * Always returns the same test card details to avoid API costs
 */
function createMockCard(
  userId: string,
  data: CreateCardRequest
): Card {
  // Generate a consistent card ID based on userId and timestamp
  const cardId = `test_card_${userId}_${Date.now()}`;
  ...
  return {
    ...
    limit: data.limit || { amount: 100000, frequency: 'perAuthorization' },
    last4: TEST_CARD.last4,
    expirationMonth: TEST_CARD.expirationMonth,
    expirationYear: TEST_CARD.expirationYear,
  };
}
 ...
 * 
 * In test mode (USE_TEST_CARDS=true), returns a mock card with test details
 * to avoid hitting the Rain API and incurring costs.
 * 
...
export async function createCard(
  ...
  // Use test card mode if enabled
  if (USE_TEST_CARDS) {
    console.log('[TEST MODE] Creating mock card instead of calling Rain API');
    return createMockCard(userId, data);
  }
...
/**
 * Create mock encrypted card secrets (used when USE_TEST_CARDS=true)
 * Returns encrypted test card PAN and CVC
 */
function createMockCardSecrets(): CardSecrets {
  // In test mode, we return mock encrypted data
  // The decryption will work because we'll handle it specially in test mode
  // For now, return base64 encoded test values
  const panBase64 = Buffer.from(TEST_CARD.pan).toString('base64');
  const cvcBase64 = Buffer.from(TEST_CARD.cvc).toString('base64');
  
  return {
    encryptedPan: {
      iv: Buffer.from('test_iv_pan_123456789012').toString('base64'), // 16 bytes
      data: panBase64,
    },
    encryptedCvc: {
      iv: Buffer.from('test_iv_cvc_123456789012').toString('base64'), // 16 bytes
      data: cvcBase64,
    },
  };
}
 ...
 * 
 * In test mode (USE_TEST_CARDS=true), returns mock encrypted test card details
 * to avoid hitting the Rain API and incurring costs.
 * 
...
export async function getCardSecrets(
  ...
  // Use test card mode if enabled
  if (USE_TEST_CARDS) {
    console.log('[TEST MODE] Returning mock card secrets instead of calling Rain API');
    return createMockCardSecrets();
  }
